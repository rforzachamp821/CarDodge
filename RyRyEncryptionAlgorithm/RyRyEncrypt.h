...
//
// RyryEncrypt.h - Contains the interface for the RyryEncrypt class.
//

// As this may change, typedef the key format
typedef uint64_t RYRYKEY;

// RyryEncrypt - Class that manages and contains an implementation of the RyRy encryption algorithm.
//
class RyRyEncrypt {
private:
	...
	// EncryptStringUsingRyryAlgorithm - Encrypts a string using the custom RyRy algorithm.
	...
	//
	bool EncryptStringUsingRyryAlgorithm(std::string* sStringToEncrypt, RYRYKEY nKey1, RYRYKEY nKey2);

	// DecryptStringUsingRyryAlgorithm - Decrypts a string that was encrypted using the custom RyRy algorithm.
	//                                 - This does NOT return a modified string, nor does it copy one into itself. 
	...
	// Return Values: TRUE or 1 for success, FALSE or 0 for fail (To get error details, call GetErrorLevelInfo()).
	//
	bool DecryptStringUsingRyryAlgorithm(std::string* sStringToDecrypt, RYRYKEY nKey1, RYRYKEY nKey2);
